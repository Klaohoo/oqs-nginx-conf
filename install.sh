...
echo "[1/10] Stopping nginx and removing old nginx packages/config"
...
rm -rf /etc/nginx
rm -rf /var/www/pq-demo
rm -rf "${LIBOQS_PREFIX}" "${OQSPROV_PREFIX}" "${LIBOQS_SRC}" "${OQSPROV_SRC}"
rm -f /etc/ld.so.conf.d/liboqs.conf
rm -f /etc/ssl/openssl-oqs.cnf
...
echo "[3/10] Building and installing liboqs"
...
{
  [[ -d "${LIBOQS_PREFIX}/lib" ]] && echo "${LIBOQS_PREFIX}/lib"
  [[ -d "${LIBOQS_PREFIX}/lib64" ]] && echo "${LIBOQS_PREFIX}/lib64"
} | awk 'NF' | sort -u >/etc/ld.so.conf.d/liboqs.conf
